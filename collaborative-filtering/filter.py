...
def movie_ids_from_users(d: Dataset, users: list[userId], watched):
    ratings = d.ratings_by_user
    ddict = defaultdict(lambda: 0.0)
    movies_map = {a.movieId: a for a in d.all_movies}
    for user in users:
        for m_id, rating in ratings[user]:
            if ddict[m_id] < rating:
                ddict[m_id] = rating
    top = sorted(ddict.items(), key=lambda item: item[1], reverse=True)
    return [movies_map[a[0]].id for a in top if movies_map[a[0]].movieId not in watched][0:10]
...
for t_user_id in test_users:
    ...
    print(top)
    ratings_testuser = d.ratings_by_user[t_user.id]
    eval_set = ratings_testuser[:len(ratings_testuser)//2]
    testset = ratings_testuser[len(ratings_testuser)//2:]
    watched = eval_set
    print(movie_ids_from_users(d, top, watched))
